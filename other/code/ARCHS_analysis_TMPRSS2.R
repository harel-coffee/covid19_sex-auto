...
#which we should removed those samples with TMPRSS2 == 0? should be OK, as long as we use the same standard for female and male

expr_meta = expr_meta[!is.na(expr_meta$TMPRSS2) & expr_meta$TMPRSS2 > 0.1, ] #& expr_meta$TMPRSS2 > 0
paste("# samples", nrow(expr_meta))

hist(log(expr_meta$TMPRSS2 + 0.01))
...
pdf(paste("figure/ARCHS_TMPRSS2_gender.pdf", sep="_"))
...
pdf(paste("figure/ARCH5_TMPRSS2_ages.pdf", sep="_"))
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((TMPRSS2)  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "TMPRSS2 Expression", side = "both", ylim=c(0,15),what = c(T, T, T, F),
...
pdf(paste("figure/ARCHS_TMPRSS2_tissue_30.pdf", sep="_"), width = 20)
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((TMPRSS2  )  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "TMPRSS2 Expression", side = "both", ylim=c(0,15), what = c(T, T, T, F),
         border = NA, col = list(c("blue","blue"), c("red", "red")), 
