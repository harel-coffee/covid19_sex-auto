...
expr_meta = read.csv("ARCHS_expr_meta.csv")
...
#should we remove weakly expressed DPP4
expr_meta = expr_meta[!is.na(expr_meta$gender) & expr_meta$DPP4 > 0.5, ] 
...
pdf(paste("figure/ARCH5_DPP4_ages.pdf", sep="_"))
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((DPP4)  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "DPP4 Expression", side = "both", ylim=c(0,15),what = c(T, T, T, F),
...
pdf(paste("figure/ARCHS_DPP4_tissue_30.pdf", sep="_"), width = 20)
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((DPP4  )  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "DPP4 Expression", side = "both", ylim=c(0,15), what = c(T, T, T, F),
         border = NA, col = list(c("blue","blue"), c("red", "red")), 
...
#####################
#investigate highly expressed DPP4
###
#airway
