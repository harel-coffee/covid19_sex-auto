#analyze DPP4 from the treehouse project
...
#describe.by(expr_meta_subset[, c("DPP4", "gender")], group = "gender")
t.test(DPP4 ~ gender, data = expr_meta)
...
p = sapply(tissues, function(x){
  ...
  if (sum(table(expr_meta_subset$gender) > 10) > 1){
    t.test(DPP4 ~ gender, data = expr_meta_subset)$p.value
  }else{
    NA
  }
})
...
##########################
#plot DPP4 expression between male and female using all data
...
pdf(paste("figure/treehouse_DPP4_ages.pdf", sep="_"))
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((DPP4)  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "DPP4 Expression", side = "both", ylim=c(-1,6),what = c(T, T, T, F),
...
pdf(paste("figure/treehouse_DPP4_tissue_30.pdf", sep="_"), width = 20)
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((DPP4 )  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "DPP4 Expression", side = "both", ylim=c(-1,6), what = c(T, T, T, F),
         border = NA, col = list(c("blue","blue"), c("red", "red")), 
