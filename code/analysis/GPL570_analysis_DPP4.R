...
symbols = c( "ESR1", "ACE2",  "GPER1", "ESR2", "AR", "PGR", "TMPRSS2")

#####################
expr_meta_subset = expr_meta
t.test(DPP4 ~ gender, data = expr_meta_subset)
anova(lm(DPP4 ~ smoke + gender, data = expr_meta_subset))
anova(lm(DPP4 ~  gender, data = expr_meta_subset))
summary(lm(DPP4 ~  gender, data = expr_meta_subset))
summary(lm(DPP4 ~  gender + age, data = expr_meta_subset))
summary(lm(DPP4 ~  gender + tissue, data = expr_meta_subset))

summary(lm(DPP4 ~    age, data = expr_meta_subset))
...
anova(lm(DPP4 ~ age, data = expr_meta_subset))
by(expr_meta_subset$DPP4, expr_meta_subset$age, mean)

#find group statistics
ages = sort(unique(expr_meta_subset$age))
test = sapply(ages, function(a){
  expr_meta_subset_age = subset(expr_meta_subset, age == a)
  Ttest = t.test(DPP4 ~ two_group, expr_meta_subset_age, alternative = "two.sided")
  paste0("ratio: ", format(Ttest$estimate[1] / Ttest$estimate[2], digit=2), ":1", "\n",
         "diff: [", format(Ttest$conf.int[1], digit=2)," ", format(Ttest$conf.int[2], digit=2), "]", "\n", 
         "p: ", format(Ttest$p.value, digits = 2, scientific=T), "\n",
         "n: ", nrow(expr_meta_subset_age)
...
pdf(paste("figure/GPL570_DPP4_ages.pdf", sep="_"))
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((DPP4)  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
...
cutoff = sort(expr_meta_subset[,gene], decreasing = T)[round(nrow(expr_meta_subset) * 0.1)] #qnorm(0.01, mean(expr_meta_subset[,gene]), sd(expr_meta_subset[, gene]), lower.tail=F)  #quantile(expr_meta[, "DPP4"])[4]
...
pdf(paste("figure/GPL570_DPP4_tissue_30.pdf", sep="_"), width = 20)
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot(DPP4   ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "DPP4 Expression", side = "both", ylim=c(0,15), what = c(T, T, T, F),
         border = NA, col = list(c("blue","blue"), c("red", "red")), 
...
Ttest = t.test(DPP4 ~ gender, expr_meta_subset, alternative = "two.sided")
...
pdf(paste("figure/GPL570_DPP4_airway.pdf", sep="_"))
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((DPP4)  ~ gender, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "DPP4 Expression", side = "both", ylim=c(0,15),what = c(T, T, T, F),
         border = NA, col = list(c("blue","blue"), c("red", "red")), 
         beanlines = "median", overalllin="median")
legend("bottomleft", fill = c("blue", "red"),
...
anova(lm(DPP4 ~ smoke, data = expr_meta_subset))
by(expr_meta_subset$DPP4, expr_meta_subset$smoke, mean)
...
test = sapply(smokes, function(a){
  ...
  Ttest = t.test(DPP4 ~ two_group, expr_meta_subset_smoke, alternative = "two.sided")
  paste0("ratio: ", format(Ttest$estimate[1] / Ttest$estimate[2], digit=2), ":1", "\n",
         "diff: [", format(Ttest$conf.int[1], digit=2)," ", format(Ttest$conf.int[2], digit=2), "]", "\n", 
         "p: ", format(Ttest$p.value, digits = 2, scientific=T), "\n",
...
pdf(paste("figure/GPL570_DPP4_smokes.pdf", sep="_"))
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((DPP4)  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "DPP4 Expression", side = "both", ylim=c(0,15),what = c(T, T, T, F),
         border = NA, col = list(c("blue","blue"), c("red", "red")), 
