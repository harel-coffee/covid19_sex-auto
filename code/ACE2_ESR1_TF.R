...
select_samples = expr_meta$sample_id[!is.na(expr_meta$tissue_old) & expr_meta$source == "gtex" & expr_meta$tissue_old == tissue & expr_meta$gender %in% c("female")]
expr_subet = all_exprs[, colnames(all_exprs) %in% select_samples]
cors = cor(t(expr_subet[c("ENSG00000130234", "ENSG00000197635", "ENSG00000159640", "ENSG00000091831"), ]), t(expr_subet[rownames(expr_subet) %in% all_genes_ensembl, ]))
...
library(pheatmap)
pheatmap(t(-log(tissue_ESR1_AR, 10)), cellheight = 10, cellwidth = 10, show_colnames=T, legend=T, show_rownames=T, cluster_cols=T, cluster_rows = F, filename = "ACE2_ESR1_AR.pdf") #, filename = "ritonavir_dz_cor_dose.pdf, filename = "ritonavir_selected_dz_cor.pdf"
