...
#describe.by(expr_meta_subset[, c("TMPRSS2", "gender")], group = "gender")
...
p = sapply(tissues, function(x){
  ...
  if (sum(table(expr_meta_subset$gender) > 10) > 1){
    t.test(TMPRSS2 ~ gender, data = expr_meta_subset)$p.value
  }else{
    NA
  }
})
...
pdf(paste("figure/treehouse_TMPRSS2_ages.pdf", sep="_"), width=10)
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((TMPRSS2)  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
...
pdf(paste("figure/treehouse_TMPRSS2_tissue_20.pdf", sep="_"), width = 30)
# par(mar = c(14, 4, 4, 2) + 0.1, las=2)
beanplot((TMPRSS2 )  ~ two_group, data = expr_meta_subset, ll = 0.15, log = "",
         main = "", ylab = "TMPRSS2 Expression", side = "both", ylim=c(-1,15), what = c(T, T, T, F),
         border = NA, col = list(c("blue","blue"), c("red", "red")), 
